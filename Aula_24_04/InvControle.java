...
public class InvControle {
    ...
    /*
    @FunctionalInterface define atualizar() como o unico metodo da interface, permitindo interpretacao 
    como callback, o que possibilita o uso de expressoes lambda no metodo main.
    */
    @FunctionalInterface
    ...
    /*
    "Subject" do Padrao Observer.
    Gerencia o estado do pedido e avisa Observers.
    */
    static class Pedido {
        // Guarda a lista de funcoes callbacks que devem ser executadas quando algo mudar.
        private List<Observador> observadores = new ArrayList<>();
        private String status;

        // Registra um callback no sistema.
        public void adicionarObservador(Observador obs) { observadores.add(obs); }
        
        public void setStatus(String status) {
            this.status = status;
            notificar(); // Sempre que o status muda, disparam os callbacks.
        }
        
        /*
        Inversao de Controle
        O Pedido chama os callbacks registrados, notificando que o status mudou.
        Ele assume o controle no lugar da main.
        */
    ...
    static class AtendenteFacade {
        ...
        // "cliente" recebe a funcao callback.
        public void processarPedido(Bebida bebida, EstrategiaPagamento pagamento, String tipoRecibo, Observador cliente) {
            ...
            if (pago) {
                ...
                // Insercao do callback no "Subject" (Pedido)
                pedido.adicionarObservador(cliente);
                
                // Estas mudancas de status disparam a Inversao de Controle.
                // O "Pedido" ira chamar o callback para avisar o cliente.
    ...
    public static void main(String[] args) {
        ...
        /*
        Uso de Lambda
        Em vez de instanciar a classe "PainelCliente", o comportamento é passado
        diretamente usando uma expressão lambda: "(status) -> { ... }". 
        */
        sistemaCaixa.processarPedido(bebidaDoCarlos, pagamentoCarlos, "APP", 
            (status) -> System.out.println("CALLBACK - O status do pedido mudou para: " + status)
        );
    }
}